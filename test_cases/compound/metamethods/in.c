...
TValue_t vector___add(TVSlice_t args) {
    ...
    _move(&v, CALL(get_tabvalue(vector, __str_ct_new_3), ((TVSlice_t){(TValue_t[]){TNUM(0), TNUM(0)}, 2})));
...
TValue_t main(TVSlice_t args) {
    ...
    _move(&v1, CALL(get_tabvalue(vector, __str_ct_new_3), ((TVSlice_t){(TValue_t[]){TNUM(5), TNUM(3)}, 2})));
    TValue_t gc v2 = T_NULL;
    _move(&v2, CALL(get_tabvalue(vector, __str_ct_new_3), ((TVSlice_t){(TValue_t[]){TNUM(1), TNUM(2)}, 2})));
    TValue_t gc delta = T_NULL;
    _move(&delta, _sub(v1, v2));
    printh(get_tabvalue(delta, __str_ct_x_1));
    printh(get_tabvalue(delta, __str_ct_y_2));
    TValue_t gc added = T_NULL;
    _move(&added, _add(v1, v2));
