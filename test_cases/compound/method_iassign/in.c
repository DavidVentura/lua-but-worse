...
TValue_t vector_len(TVSlice_t args) {
    ...
    return _sqrt(_add(_mult(get_tabvalue(self, __str_ct_x_1), get_tabvalue(self, __str_ct_x_1)), _mult(get_tabvalue(self, __str_ct_y_2), get_tabvalue(self, __str_ct_y_2))));
}
...
TValue_t main(TVSlice_t args) {
    TValue_t gc zero;
    _set(&zero, CALL(get_tabvalue(vector, __str_ct_new_6), ((TVSlice_t){(TValue_t[]){TNUM(0), TNUM(0)}, 2})));
    printh(CALL(get_tabvalue(zero, TSTR("len")), ((TVSlice_t){(TValue_t[]){zero}, 1})));
    TValue_t gc one;
    _set(&one, CALL(get_tabvalue(vector, __str_ct_new_6), ((TVSlice_t){(TValue_t[]){TNUM(1), TNUM(1)}, 2})));
