...
TValue_t main(TVSlice_t args) {
    ...
    _move(&zero, CALL(get_tabvalue(vector, __str_ct_new_2), ((TVSlice_t){(TValue_t[]){TNUM(0), TNUM(0)}, 2})));
    TValue_t gc one = T_NULL;
    _move(&one, CALL(get_tabvalue(vector, __str_ct_new_2), ((TVSlice_t){(TValue_t[]){TNUM(1), TNUM(1)}, 2})));
